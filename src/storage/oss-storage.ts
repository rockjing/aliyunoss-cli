...
export class OSSStorageService implements StorageService {
  ...
  async uploadFile(content: Buffer, filename: string, options?: UploadOptions): Promise<string> {
    try {
      ...
      const result = await this.client.put(filename, content, uploadOptions);
      ...
      this.log('error', '文件上传失败', { filename, error });
      
      if (this.isTimeoutError(error)) {
        throw new MCPError(
          ErrorCode.OSS_TIMEOUT_ERROR,
          `文件上传超时: ${filename}`,
          { filename, timeout: this.config.timeout, error }
        );
      }
  ...
  async generateTempUrl(filename: string, expires?: number): Promise<string> {
    try {
      ...
      this.log('info', '生成临时下载链接', { filename, expires: expiresIn });
      ...
      const duration = Date.now() - startTime;
      this.log('info', '临时链接生成成功', { filename, duration });
      
      return url;
    } catch (error) {
      this.log('error', '生成临时下载链接失败', { filename, error });
      throw new MCPError(
        ErrorCode.OSS_CONNECTION_ERROR,
        `生成临时下载链接失败: ${filename}`,
        { filename, error }
  ...
  async deleteFile(filename: string): Promise<void> {
    try {
      ...
      const duration = Date.now() - startTime;
      this.log('info', '文件删除成功', { filename, duration });
    } catch (error) {
      this.log('error', '文件删除失败', { filename, error });
      
      if (this.isNotFoundError(error)) {
        // 文件不存在时不抛出错误，认为删除成功
        this.log('warn', '删除的文件不存在', { filename });
        return;
      }
  ...
  async getFileContent(filename: string): Promise<Buffer> {
    try {
      ...
      this.log('error', '获取文件内容失败', { filename, error });
  ...
  async copyFile(source: string, target: string): Promise<void> {
    try {
      this.log('info', '复制文件', { source, target });
      ...
      const duration = Date.now() - startTime;
      this.log('info', '文件复制成功', { source, target, duration });
    } catch (error) {
      this.log('error', '文件复制失败', { source, target, error });
      throw new MCPError(
        ErrorCode.OSS_CONNECTION_ERROR,
        `文件复制失败: ${source} -> ${target}`,
        { source, target, error }
      );
    }
  }
  ...
  async getFileMetadata(filename: string): Promise<FileMetadata> {
    try {
      this.log('info', '获取文件元数据', { filename });
      ...
      const duration = Date.now() - startTime;
      this.log('info', '文件元数据获取成功', { filename, duration });
      ...
      this.log('error', '获取文件元数据失败', { filename, error });
  ...
  async setFileACL(filename: string, acl: string): Promise<void> {
    try {
      this.log('info', '设置文件权限', { filename, acl });
      
      const startTime = Date.now();
      
      await this.client.putACL(filename, acl as any, {
        timeout: this.config.timeout ? this.config.timeout * 1000 : 60000
      });
      
      const duration = Date.now() - startTime;
      this.log('info', '文件权限设置成功', { filename, acl, duration });
    } catch (error) {
      this.log('error', '设置文件权限失败', { filename, acl, error });
      throw new MCPError(
  ...
  async getFileACL(filename: string): Promise<ACLResult> {
    try {
      this.log('info', '获取文件权限', { filename });
      
      const startTime = Date.now();
      
      const result = await this.client.getACL(filename, {
        timeout: this.config.timeout ? this.config.timeout * 1000 : 60000
      });
      
      const duration = Date.now() - startTime;
      this.log('info', '文件权限获取成功', { filename, duration });
      ...
      this.log('error', '获取文件权限失败', { filename, error });
      throw new MCPError(
        ErrorCode.OSS_PERMISSION_ERROR,
        `获取文件权限失败: ${filename}`,
        { filename, error }
  ...
  async deleteMultipleFiles(filenames: string[]): Promise<BatchDeleteResult> {
    try {
      this.log('info', '批量删除文件', { count: filenames.length });
      ...
      this.log('error', '批量删除文件失败', { filenames, error });
      throw new MCPError(
        ErrorCode.FILE_DELETE_FAILED,
        '批量删除文件失败',
        { filenames, error }
  ...
  async initMultipartUpload(filename: string, options?: UploadOptions): Promise<string> {
    try {
      this.log('info', '初始化分片上传', { filename });
      ...
      const result = await this.client.initMultipartUpload(filename, uploadOptions);
      ...
      this.log('error', '初始化分片上传失败', { filename, error });
      throw new MCPError(
  ...
  async uploadPart(filename: string, uploadId: string, partNumber: number, content: Buffer): Promise<PartResult> {
    try {
      ...
      const result = await this.client.uploadPart(filename, uploadId, partNumber, content, 0, content.length, {
        timeout: this.config.timeout ? this.config.timeout * 1000 : 300000
      });
      ...
      this.log('error', '分片上传失败', { filename, uploadId, partNumber, error });
      throw new MCPError(
        ErrorCode.MULTIPART_UPLOAD_FAILED,
        `分片上传失败: ${filename} 分片${partNumber}`,
        { filename, uploadId, partNumber, error }
      );
    }
  }
  ...
  async completeMultipartUpload(filename: string, uploadId: string, parts: PartInfo[]): Promise<CompleteMultipartResult> {
    try {
      ...
      const result = await this.client.completeMultipartUpload(filename, uploadId, parts, {
        timeout: this.config.timeout ? this.config.timeout * 1000 : 300000
      });
      ...
      return {
        ...
        key: (result as any).key || filename,
        etag: result.etag,
        size: parts.reduce((total, part) => total + (part.size || 0), 0)
      };
    } catch (error) {
      this.log('error', '完成分片上传失败', { filename, uploadId, error });
      throw new MCPError(
  ...
  async abortMultipartUpload(filename: string, uploadId: string): Promise<void> {
    try {
      this.log('info', '取消分片上传', { filename, uploadId });
      
      const startTime = Date.now();
      
      await this.client.abortMultipartUpload(filename, uploadId, {
        timeout: this.config.timeout ? this.config.timeout * 1000 : 60000
      });
      
      const duration = Date.now() - startTime;
      this.log('info', '分片上传已取消', { filename, uploadId, duration });
    } catch (error) {
      this.log('error', '取消分片上传失败', { filename, uploadId, error });
      throw new MCPError(
        ErrorCode.OSS_CONNECTION_ERROR,
        `取消分片上传失败: ${filename}`,
        { filename, uploadId, error }
  ...
  async listMultipartUploads(options?: { prefix?: string; maxUploads?: number; keyMarker?: string; uploadIdMarker?: string; }): Promise<MultipartListResult> {
    try {
      this.log('info', '列出进行中的分片上传', { options });
